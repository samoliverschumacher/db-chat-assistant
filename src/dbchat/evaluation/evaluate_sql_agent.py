...
def test_faithfulness():
    ...
    evaluator = llama_index_faithfulness.FaithfulnessEvaluator(
        service_context=service_context)
...
def test_faithfulness_with_query():
    ...
    service_context = ServiceContext.from_defaults(
        llm_predictor=LLMPredictor(llm=judge_llm), embed_model=embedding_model)

    # define evaluator: whether the response is supported by the contexts or hallucinated
    evaluator = llama_index_relevancy.RelevancyEvaluator(
        service_context=service_context)
